...
Fout zoals `ERROR: Script file '' does not exist.` bij PyInstaller?
- De workflows gebruiken nu `./scripts/build_windows.ps1` als centrale buildstap (geen lege `$iconArg` meer).
- Als je die oude fout nog ziet, draait je pipeline nog op een oudere commit/branch. Run opnieuw op de nieuwste commit.
