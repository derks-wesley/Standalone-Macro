...
class TestPackagingFiles(unittest.TestCase):
    ...
    def test_windows_ci_workflow_exists(self):
        ...
        self.assertIn("./scripts/build_windows.ps1", content)
    ...
    def test_build_script_uses_argument_array(self):
        script = self.root / "scripts" / "build_windows.ps1"
        self.assertTrue(script.exists())
        content = script.read_text(encoding="utf-8")
        self.assertIn("$pyiArgs = @(", content)
